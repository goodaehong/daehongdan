...
평면도 이미지(PNG)를 STM32 HUB75 디스플레이용 64x64 대피도 데이터(C 배열)로 변환하는 빌드타임 도구.
...
```c
// 0=빈공간(통행가능), 1=장애물
const uint8_t HUB75_EvacMap[64][64] = { ... };
...
const uint8_t EvacExits[][2] = {      // 출구 위치들
  {50,63}, {0,50}, {63,16}, {22,0}
};
#define EVAC_EXIT_COUNT 4
```

- **좌표계**: `[x][y]` 아님, `[y][x]`(행 우선) 배열이지만 좌표 자체는 `{x, y}` 순서. `HUB75_EvacMap[y][x]`로 접근.
...
- **`EvacExits[i][0]`=x, `[i][1]`=y**: 항상 `x==0`, `x==63`, `y==0`, `y==63` 중 하나 (바깥 경계선 위).
...
- 목표: 특정 전광판 위치(`EvacDisplays[i]`)에서 `EvacExits[]` 중 가장 가까운 출구까지 BFS로 최단 경로 탐색 예정 (그리드가 64x64 균일 비용이라 BFS가 Dijkstra/A*보다 단순하고 충분함).
- 화재/위험구역 표시는 아직 미구현. 기존 알림 패킷(CMD 0x90)의 `zoneID` 필드(카메라 채널 `detCh` 매핑)를 재사용해서, 특정 구역을 동적으로 장애물 취급하는 방식으로 확장할 계획.
- `image_to_bitmap.cpp`에 `kManualWalls`(수동 벽 보정 목록)가 정의는 돼있지만 `main()`에서 실제로 적용되진 않음 — 현재는 원본 이미지에서 얇은 벽을 직접 두껍게 그려서 해결했기 때문에 미사용 상태. 필요 없으면 지워도 됨.
