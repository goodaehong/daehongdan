...
- 평상시 화면(가스 농도 · 온습도 · 표정 · 시각)과 대피 화면 두 가지
...
```
CMD_UPDATE(0x80)      평상시 화면 갱신 (1초 주기)
CMD_ALERT(0x90)       대피 화면 전환 — 화재 / 가스
CMD_EVAC_FIRES(0xB2)  화재 위치 목록 (최대 6곳)
CMD_EVAC_PATH(0xB1)   대피경로 (출구당 1패킷, 웨이포인트 최대 30)
CMD_CLEAR(0xA0)       평상 복귀
CMD_ACK(0xB0)         상태 응답 (STM32 → Pi)
```
...
**경로를 2단계로 그립니다.** ① 전체 경로를 빨강으로 다 그리고 ② 하이라이트를 그 위에 얹습니다.
경로별로 번갈아 그리면 뒤 경로의 빨간칠이 앞 경로의 하이라이트를 덮습니다.

**화재 좌표는 사각형의 「아랫변 중앙」입니다.** 카메라 쪽에서 화염 박스의 바닥 접촉점을
대표 위치로 잡아 넘기기 때문입니다. 그래서 세로는 `-2r ~ 0` 으로 **위로만** 뻗습니다.

**불을 흔듭니다.** 완전히 정적인 도형은 저해상도에서 「과녁」처럼 보입니다.
50ms마다 노란 코어의 크기와 위치를 ±1 흔들어 불처럼 읽히게 합니다.
...
| 파일 | 역할 |
| --- | --- |
| `Core/Src/main.c` | UART 수신 · 명령 처리 · 화면 상태 · `EVAC_DATA` 지도 배열 |
| `Core/Src/hub75_display.c` | HUB75 패널 스캔 · 픽셀 버퍼 |
...
`HUB75_EvacMap[60][60]` · `EvacDisplays[][2]` · `EvacExits[][2]` 가 함께 갱신됩니다.
마커를 못 찾으면 아무것도 건드리지 않고 실패를 알립니다.

> ⚠️ CubeMX 로 코드를 재생성하면 `USER CODE` 구간 밖은 사라집니다.
> `EVAC_DATA` 는 그 구간 안에 있어야 보존됩니다.
