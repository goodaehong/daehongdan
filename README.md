...
- 화재 박스의 바닥 접촉점을 ArUco 기준 **공장 좌표(60×60)** 로 변환해 전광판·대피경로와 좌표계 공유
...
│       ├── widgets/             재사용 위젯 (그래프 · 감지 오버레이 · ROI 편집 · ArUco 보정)
...
| 김유나 | Qt 관제 클라이언트 · 커널 드라이버 · 센서 · Qt TLS |
| 마재환 | 화재 · 연기 감지 코어 · 모델 학습 |
| 김광렬 | 전광판 펌웨어 · TLS 서버 |
| 장태호 | 액추에이터 펌웨어 · 대피경로 알고리즘 |
