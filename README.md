...
![시스템 구성도](.github/images/system-architecture.png)

- 카메라 4대 · 센서 4종 → 라즈베리파이 서버가 통합 판단
- 판단 결과 → STM32 보드 2종(액추에이터 · 전광판) · 음성 · 관제 화면
- 서버와 관제 화면은 TLS 로 통신
...
![데이터 흐름도](.github/images/data-flow.png)

감지 → 판단 → 대응 → 기록 4단계로 흐르고, 설정 연동과 보안 통신이 전 단계를 가로지른다.
