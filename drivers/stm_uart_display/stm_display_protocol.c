                                      ...
                                      const uint8_t *waypointsXY, uint8_t waypointCount)
{
    ...
    /* data[0..3] = fireX,fireY,fireRadius,waypointCount, 그 뒤로 {x,y} 반복
       - main.c HandlePacket()의 CMD_EVAC_PATH 파싱 순서와 정확히 일치해야 함 */
    uint8_t data[4 + STM_DISPLAY_EVAC_MAX_WAYPOINTS * 2];
    data[0] = fireX;
    data[1] = fireY;
    data[2] = fireRadius;
    data[3] = waypointCount;
    memcpy(&data[4], waypointsXY, (size_t)waypointCount * 2);

    return SendPacket(fd, STM_DISPLAY_CMD_EVAC_PATH, data, (uint8_t)(4 + waypointCount * 2));
}
