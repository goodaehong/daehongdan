...
static void PrintMenu()
{
    std::cout <<
        ...
        "  3 : 화재 1곳 전송(0xB2) - (5,5) 반경4\n"
        "  4 : 화재 2곳 전송(0xB2) - (5,5)반경4, (40,20)반경3\n"
        "  5 : 화재 6곳(최대치) 전송(0xB2) + 그 화재 기준 실제 재계산 경로 전송(0xB1x4) - 화재를 피해 우회하는 경로 확인용\n"
...
static void InputWorker()
{
    ...
    while (std::getline(std::cin, line))
    {
        ...
        else if (line == "5")
        {
            ...
            // (8,45)는 원래 전광판3(36,11) 위치를 화재 위험구역이 통째로 덮어서 4개 출구 전부
            // 고립되던 좌표였음 - (8,52)로 옮겨서 전광판은 안 덮으면서 실제로 우회하는 경로가 나오게 함
