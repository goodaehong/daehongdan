...
$output = Join-Path $opencvDirectory ("camera_calibration_ch{0}.yml" -f $Channel)
...
try {
    ...
    Write-Host "Channel $Channel ChArUco calibration"
    Write-Host "Board: DICT_4X4_50, 7x5, square 50 mm, marker 35 mm"
    Write-Host "SPACE=capture, U=undo, C=finish/save, Q=quit"
    Write-Host "Move and tilt the board; collect 20-25 diverse views."
    & $executable $cameraUrl --channel $Channel --output $output
