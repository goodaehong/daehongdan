...
void drawDetectionResult(Mat& display, const DetectionResult& result)
{
    ...
    {
        const Scalar color = detection.type == DetectionType::FIRE ? Scalar(0, 0, 255) : Scalar(255, 255, 0);
        rectangle(display, detection.box, color, 2);
        putText(display, detection.label, Point(detection.box.x, max(20, detection.box.y - 8)),
...
bool FireView::show(const Mat& frame, const FireRuntimeSnapshot& snapshot, double displayFps)
{
    ...
    if (snapshot.boxIsFresh) drawDetectionResult(displaySource, snapshot.detection);
    ...
    putText(display, stateText, Point(20, 40), FONT_HERSHEY_SIMPLEX, 1.0, stateColor, thickness);
    ...
    putText(display, performanceText, Point(20, 80), FONT_HERSHEY_SIMPLEX, 0.60, Scalar(255, 255, 255), 2);
...
void FireView::close() const { destroyAllWindows(); }
...
Mat FireView::makeDebugTile(const Mat& source, const string& title) const
{
    ...
    Mat gray;
    if (source.empty()) gray = Mat::zeros(tileSize, CV_8UC1);
    else
    {
        if (source.channels() == 1) gray = source;
        else cvtColor(source, gray, COLOR_BGR2GRAY);
        if (gray.size() != tileSize) resize(gray, gray, tileSize, 0, 0, INTER_NEAREST);
    }

    Mat tile;
    cvtColor(gray, tile, COLOR_GRAY2BGR);
    putText(tile, title, Point(12, 28), FONT_HERSHEY_SIMPLEX, 0.70, Scalar(0, 255, 255), 2);
    return tile;
}
