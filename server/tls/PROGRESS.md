...
- **라즈베리파이 실빌드 + 실동작 검증 완료 (2026-08-21)**
  ...
  - ⚠️ 단일 접속 차단 테스트 중 **버그 발견** — 아래 남은 과제 1번 참고

## 남은 과제

1. **[버그] 단일 접속(1:1) 차단 로직이 동시 접속 상황에서 실제로 작동하지 않음** (2026-08-21 검증 중 발견)
   - **증상**: 첫 연결이 살아있는 상태에서 두 번째 클라이언트가 접속 시도하면, `[TLS] 다중 연결 시도 차단` 로그가 전혀 안 뜨고, 두 번째 클라이언트는 TCP `CONNECTED`까지만 뜬 채로 아무 응답 없이 무한 대기(hang)함. (결과적으로 두 번째 접속이 서비스되지는 않으니 "차단"은 되고 있지만, 의도한 로그/명시적 거부 처리 경로를 안 탐)
   - **원인**: `run()`의 `accept()`가 바깥쪽 while 루프에서 한 번만 호출되고, 그 직후 안쪽 while 루프(poll+SSL_read)로 들어가 **첫 연결이 끊길 때까지 거기서 안 나옴**. 그래서 `if (ssl_) { 다중 연결 차단 로그 }` 체크가 있는 코드 지점 자체에 두 번째 연결 동안은 도달하지 못함. `link_tls.cpp` 결합 이전부터 있던 구조적 문제(90187fd 리팩터링 때부터), 이번 작업에서 새로 만든 버그는 아님.
   - **제안하는 수정**: 안쪽 RX 루프의 `poll()`이 지금 `clientFd` 하나만 감시하는데, 여기에 `serverFd_`(리스닝 소켓)도 같이 넣어서, 첫 연결을 서비스하는 도중에도 새 접속 시도를 즉시 감지 → `accept()` → 바로 `close()` + 로그 출력하도록 고치면 됨. (poll의 두 번째 fd 슬롯 추가 + `revents` 분기만 있으면 되는 비교적 작은 수정)
   - 아직 코드 수정은 안 한 상태 — 다음 작업 우선순위로 처리 필요
2. **인증서 경로(`server_cert.pem`/`server_private.pem`) 상대경로 하드코딩** — 실행 CWD에 의존. 다른 항목들과 달리 이건 라즈베리파이의 실제 배포 경로/관례를 모르는 채로 손대면 지금 되던 걸 깨뜨릴 위험이 있어 일부러 안 건드림. 실제 배포 시 인증서가 어디 놓이는지 확인 후 `FLOORMAP_IMAGE_PATH`처럼 `CMakeLists.txt` compile definition으로 절대경로화하는 걸 권장. 실제 발급 절차는 `server/tls/QT_TLS_전환_가이드.md` 참고
3. **Qt 클라이언트 값 미적용** — `kUseTls`/인증서 지문/`kServerHost` 세 값을 아직 실제로 안 바꿈. 적용할 값과 주의사항은 `server/tls/QT_TLS_전환_가이드.md`에 정리해둠
