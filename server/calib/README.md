...
### 불완전한 좌표 보정 결과가 운영에 적용되던 문제

마커 대응점이 부족하거나 오검출이 포함된 상태에서 계산한 좌표 변환식은 화재 위치를
잘못된 공장 좌표로 전달할 수 있었다. 설정된 마커가 4개 이상 검출된 경우에만 계산을
진행하고, RANSAC으로 이상 대응점을 제외한 뒤 품질 조건을 통과한 Homography만
채널별 파일에 저장하도록 변경했다. 보정 작업도 한 번에 한 채널만 수행하게 해 RTSP
입력과 결과 파일이 서로 섞이지 않도록 했으며, 서버는 검증된 최신 파일만 다시 불러와
운영 좌표에 적용하도록 해결했다.
