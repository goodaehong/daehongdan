...
### 카메라마다 공장 영역의 네 모서리를 수동으로 지정하던 문제

화면의 네 점을 사람이 직접 선택하는 방식은 카메라가 여러 대일 때 같은 작업을
반복해야 하고, 바닥만 보이는 영상에서는 동일한 실제 위치를 다시 찾기 어려웠다.
먼저 ChArUco 보드로 렌즈 왜곡을 보정하고, ID별 공장 XY 좌표가 등록된 ArUco
마커 중심을 영상 좌표와 자동 대응시켰다. 오검출 대응점은 RANSAC으로 제외한 뒤
Homography를 계산해 채널별 파일로 저장하도록 변경했다. 운영 중에는 저장된 행렬을
불러오므로 매 프레임 마커를 검출하지 않고도 화재 위치를 60×60 디스플레이 좌표로
변환할 수 있다.
