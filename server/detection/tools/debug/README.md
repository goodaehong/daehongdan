...
| 증상 | 확인할 것 |
| --- | --- |
| 연기 모델을 못 읽음 | 실행 파일 옆 모델 폴더와 NCNN DLL 확인 |
| 연기 박스가 없음 | 416×256 입력, confidence 0.40, class 0 확인 |
| ArUco가 OFF | 설정 파일에 해당 채널 `BOARD`·`MARKER`가 있는지 확인 |
| 중심점 인라이어가 4 미만 | 설정 ID의 가시 여부와 좌표 분포 확인 |
| 화재의 `grid=(x,y)`가 없음 | Homography 유효·최신 상태와 카메라 보정 파일 확인 |
